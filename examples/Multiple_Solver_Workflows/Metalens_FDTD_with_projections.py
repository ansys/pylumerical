...
zemax_coeffs = [-2.279343664281709E+006,-3.943219031309796E+006,8.648523491639540E+009,-1.743623426550141E+012]
...
# Part 2: Create simple unit cell libary.
...
with lumapi.FDTD(hide = False) as rcwa:
    for n in range(0,sweep_res):
        ...
        phase_results[n], amp_results[n] = single_RCWA(rcwa, 'circle', radius, period, height, pillar_material, substrate_material, target_wavelength, 0, 0)
...
with lumapi.FDTD(hide = False) as fdtd:
    ...
    if assume_symmetry:
        ...
        # No symmetry - build array exacty
    ...
    fdtd_geometry_props = {"x": 0, "x span" : 2*(lens_radius + buffer_space), "y": 0, "y span" : 2*(lens_radius + buffer_space), "z min": -buffer_space, "z max" : height + buffer_space}
    fdtd_boundary_props = {"x min bc" : "Anti-Symmetric", "x max bc": "PML", "y min bc" : "Symmetric", "y max bc": "PML", "z min bc": "PML", "z max bc": "PML"}
    ...
    # Plane wave source
    source_geometry_props = {"injection axis": "z", "direction": "forward", "x": 0, "x span": 3*lens_radius, "y": 0, "y span": 3*lens_radius, "z": -target_wavelength}
    ...
    monitor_geometry_props = {"name": "field_above", "monitor type": "2D Z-normal", "x": 0, "x span": 3*lens_radius, "y": 0, "y span": 3*lens_radius, "z": height+buffer_space/2}
    monitor_frequency_props = {"override global monitor settings": 1, "use source limits": 0, "minimum wavelength": target_wavelength, "maximum wavelength": target_wavelength, "frequency points": 1}
    ...
    monitor_props = {"name": "side_view", "monitor type": "2D Y-normal", "x": 0, "x span": 3*lens_radius, "y": 0, "z min": -buffer_space, "z max": height + buffer_space}
    fdtd.adddftmonitor(properties = monitor_props)
