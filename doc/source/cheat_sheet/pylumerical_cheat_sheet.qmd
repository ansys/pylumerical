...
```{python}
with lumapi.FDTD(hide=True) as fdtd:
  runMode = "GPU" # Use "CPU" for CPU simulation, use "GPU" for GPU simulation
...
with lumapi.FDTD(hide =True) as fdtd:
  ...
  # Extract result from Lumerical dataset
  ...
  # Can also use fdtd.getelectric(), fdtd.getmagnetic() to get amplitude quickly
  electricFieldAmplitude = Ex ** 2 + Ey ** 2 + Ez ** 2
  # Plot with Matplotlib
  X, Y = np.meshgrid(x, y)  # Create meshgrid
  plt.contourf(X, Y, electricFieldAmplitude, cmap='jet')
```